    ...
    public AttackRequirement(final Card attacker, final MapToAmount<Card> causesToAttack, final FCollectionView<GameEntity> possibleDefenders) {
        ...
        for (final GameEntity entity : combinedDefMap.keySet()) {
            ...
            if (entity instanceof Player) {
                ...
                final Player controller = reqPW.getController();
                if (!controller.isInGame()) {
                    removeThis = true;
                } else {
                    final Card gamePW = game.getCardState(reqPW, null);
                    if (gamePW == null || !gamePW.equalsWithGameTimestamp(reqPW) || !gamePW.getController().isOpponentOf(attacker.getController())) {
                        removeThis = true;
                    }
                }
            }
